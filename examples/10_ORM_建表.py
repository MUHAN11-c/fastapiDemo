"""
=== 第10课：ORM 建表（SQLAlchemy ORM）===

ORM = Object Relational Mapping（对象关系映射）喵~
用 Python 类表示数据库表，用对象表示表中的一行数据喵~

三步走：
1. 创建异步引擎（Engine）→ 管理数据库连接池喵~
2. 定义模型类（Model）→ Python类映射数据库表喵~
3. 启动时建表（create_all）→ 根据模型类自动创建表结构喵~
"""
